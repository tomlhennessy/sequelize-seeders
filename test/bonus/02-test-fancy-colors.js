...
describe('BONUS Seeder Specs - Fancy Colors', () => {
  let DB_TEST_FILE, SERVER_DB_TEST_FILE, models, server, seedFile;
  before(async () => {
  ...
  it('has at least 1 entry in the Colors table after committing fancy color seeder file', async () => {
    const colors = await runSQLQuery("SELECT * FROM 'Colors' WHERE name NOT IN ('red', 'yellow', 'blue')", SERVER_DB_TEST_FILE);
  ...
  it('has a down function that deletes ONLY the colors created in the up function', async () => {
    await runSQL("INSERT INTO 'Colors'(name) VALUES ('test color')", SERVER_DB_TEST_FILE);
    const colors = await runSQLQuery("SELECT * FROM 'Colors' WHERE name NOT IN ('red', 'yellow', 'blue')", SERVER_DB_TEST_FILE);
    expect(colors).to.have.lengthOf.at.least(2);
    await expect(undoAllSeeds(DB_TEST_FILE)).to.not.eventually.be.rejectedWith(Error);
    const allColors = await runSQLQuery("SELECT * FROM 'Colors' WHERE name NOT IN ('red', 'yellow', 'blue')", SERVER_DB_TEST_FILE);
